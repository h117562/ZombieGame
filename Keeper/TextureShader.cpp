...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	result = InitializeShader(device, hwnd, L"../PixelShader.hlsl", L"../VertexShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, XMMATRIX modelPosition,
	XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	...
	result = SetShaderParameters(deviceContext, modelPosition, viewMatrix, projectionMatrix);
	if (!result)
	{
		return false;
	}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* psFilename, WCHAR* vsFilename) 
{
	HRESULT result;
	...
	D3D11_BUFFER_DESC matrixBufferDesc, boneBufferDesc;
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, vsFilename, L"Missing TEXTUREShader File", MB_OK);
		}
