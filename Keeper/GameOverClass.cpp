...
bool GameOverClass::Initialize(D3DClass* Direct3D, HWND hwnd, string directory, int screenWidth, int screenHeight)
{
	bool result;
	...
	result = m_ContinueImage->Initialize(Direct3D->GetDevice(), hwnd, directory, "data\\Model\\Button\\Gotomain.png", XMFLOAT3(0.0f, -50, 1.0f), 4.0f);
	if (!result)
	{
	...
	result = m_ContinueImage_S->Initialize(Direct3D->GetDevice(), hwnd, directory, "data\\Model\\Button\\Gotomains.png", XMFLOAT3(0.0f, -50, 1.0f), 4.0f);
	if (!result)
	{
	...
	result = m_PauseImage->Initialize(Direct3D->GetDevice(), hwnd, directory, "data\\Model\\Button\\GameOver.png", XMFLOAT3(0, 150, 0), 4);
	if (!result)
	{
		return false;
	}
	...
	result = m_BackGround->Initialize(Direct3D->GetDevice(), hwnd, directory, "data\\Model\\Button\\BackGround2.png", XMFLOAT3(0, 0, 0), 50);
	if (!result)
	{
		return false;
	}
...
bool GameOverClass::Frame(D3DClass* Direct3D, InputClass* Input, ShaderManagerClass* ShaderManager,
...
bool GameOverClass::HandleMovementInput(InputClass* Input, float frameTime, int fps, bool& b_continue)
{
	bool result;
	bool keyDown;
	bool keyDown2;
...
bool GameOverClass::ButtonEvent() {
...
bool GameOverClass::Render(D3DClass* Direct3D, ShaderManagerClass* ShaderManager, ImageShaderClass* ImageShader)
{
